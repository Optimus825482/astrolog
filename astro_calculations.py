    ...
    try:
        ...
        if main_ruler and sub_ruler and main_period_start_date and main_period_end_date and sub_period_start_date and sub_period_end_date:
            ...
            return {
                ...
                "sub_period_start_date": sub_period_start_date.strftime("%Y-%m-%d") if sub_period_start_date else "N/A",
                "sub_period_end_date": sub_period_end_date.strftime("%Y-%m-%d") if sub_period_end_date else "N/A",
...
def calculate_solar_return_chart(birth_dt, current_dt, latitude, longitude):
    ...
    try:
        ...
        solar_return_asc_degree = solar_return_houses_data.get("important_angles", {}).get("ascendant")
        solar_return_asc_sign = get_zodiac_sign(solar_return_asc_degree) if solar_return_asc_degree is not None else "Bilinmiyor"
        solar_return_asc_deg_in_sign = round(get_degree_in_sign(solar_return_asc_degree), 2) if solar_return_asc_degree is not None else 0.0
        ...
        solar_return_chart_data = {
            ...
            "datetime": solar_return_dt.strftime("%Y-%m-%d %H:%M:%S"), # Geriye dönük uyumluluk için
...
def calculate_lunar_return_chart(birth_dt, current_dt, latitude, longitude):
    ...
    try:
        ...
        # Sonuç sözlüğünü oluştur
        # Yükselen burç ve derecesini al
        lunar_return_asc_degree = lunar_return_houses_data.get("important_angles", {}).get("ascendant")
        lunar_return_asc_sign = get_zodiac_sign(lunar_return_asc_degree) if lunar_return_asc_degree is not None else "Bilinmiyor"
        lunar_return_asc_deg_in_sign = round(get_degree_in_sign(lunar_return_asc_degree), 2) if lunar_return_asc_degree is not None else 0.0
        ...
        lunar_return_chart_data = {
            ...
            "datetime": lunar_return_dt.strftime("%Y-%m-%d %H:%M:%S"), # Geriye dönük uyumluluk için
            "location": {"latitude": latitude, "longitude": longitude},
...
def find_eclipses_in_range(start_dt, end_dt):
    ...
    try:
        ...
        actual_start_dt = datetime(min_valid_year, 1, 1) if start_dt.year < min_valid_year else start_dt
        
        # Bitiş tarihi çok uzak gelecekteyse sınırla
        max_valid_year = min(end_dt.year, 3000)  # Maksimum 3000 yıl olarak sınırla
        actual_end_dt = datetime(max_valid_year, 12, 31) if end_dt.year > max_valid_year else end_dt
        ...
        solar_eclipse_dates = [
            datetime(2020, 6, 21), datetime(2020, 12, 14),
            datetime(2021, 6, 10), datetime(2021, 12, 4),
            datetime(2022, 4, 30), datetime(2022, 10, 25),
            datetime(2023, 4, 20), datetime(2023, 10, 14),
            datetime(2024, 4, 8), datetime(2024, 10, 2),
            datetime(2025, 3, 29), datetime(2025, 9, 21),
            datetime(2026, 2, 17), datetime(2026, 8, 12),
            datetime(2027, 2, 6), datetime(2027, 8, 2),
            datetime(2028, 1, 26), datetime(2028, 7, 22),
            datetime(2029, 1, 14), datetime(2029, 7, 11),
            datetime(2029, 12, 5), datetime(2030, 6, 1),
            datetime(2030, 11, 25)
        ]
        
        lunar_eclipse_dates = [
            datetime(2020, 1, 10), datetime(2020, 6, 5), datetime(2020, 7, 5), datetime(2020, 11, 30),
            datetime(2021, 5, 26), datetime(2021, 11, 19), 
            datetime(2022, 5, 16), datetime(2022, 11, 8),
            datetime(2023, 5, 5), datetime(2023, 10, 28),
            datetime(2024, 3, 25), datetime(2024, 9, 18),
            datetime(2025, 3, 14), datetime(2025, 9, 7),
            datetime(2026, 3, 3), datetime(2026, 8, 28),
            datetime(2027, 2, 20), datetime(2027, 8, 17),
            datetime(2028, 2, 10), datetime(2028, 8, 6),
            datetime(2029, 1, 30), datetime(2029, 7, 26),
            datetime(2030, 1, 20), datetime(2030, 7, 15)
        ]
        ...
        for solar_date in solar_eclipse_dates:
            if actual_start_dt <= solar_date + timedelta(days=40) and solar_date - timedelta(days=40) <= actual_end_dt:
                all_eclipse_triggers.append((solar_date, "Solar"))
                
        for lunar_date in lunar_eclipse_dates:
            if actual_start_dt <= lunar_date + timedelta(days=40) and lunar_date - timedelta(days=40) <= actual_end_dt:
        ...
        for trigger_date, eclipse_type in all_eclipse_triggers:
            ...
            jd_start = swe.julday(search_start.year, search_start.month, search_start.day, 0)
            jd_end = swe.julday(search_end.year, search_end.month, search_end.day, 23.99)
            ...
            try:
                ...
                if eclipse_type == "Lunar":
                    ...
                    if res and res[0] < jd_end:
                        ...
                        if dt_parts and dt_parts[0] > 0 and dt_parts[0] < 9999:
                            ...
                            type_name = "Total" if ecl_type & swe.ECL_TOTAL else \
                                       "Partial" if ecl_type & swe.ECL_PARTIAL else \
                                       "Penumbral" if ecl_type & swe.ECL_PENUMBRAL else "Unknown"
                ...
                if eclipse_type == "Solar":
                    ...
                    if res and res[0] < jd_end:
                        ...
                        if dt_parts and dt_parts[0] > 0 and dt_parts[0] < 9999:
                            ...
                            # Tutulma tipini belirle
                            type_name = "Total" if ecl_type & swe.ECL_TOTAL else \
                                       "Annular" if ecl_type & swe.ECL_ANNULAR else \
                                       "Partial" if ecl_type & swe.ECL_PARTIAL else \
                                       "Annular-Total" if ecl_type & swe.ECL_ANNULAR_TOTAL else "Unknown"
                ...
                logger.warning(f"Tutulma arama hatası (tarih: {trigger_date}): {str(e)}")
                continue
        ...
        logger.info(f"Tutulma arama tamamlandı. Bulunan tutulma sayısı: {len(unique_eclipses)}")
        ...
        for i in range(len(points_keys)):
            for j in range(i + 1, len(points_keys)): # Çiftleri tekrar etmeden al
                ...
                for p3_key, deg3 in normalized_positions.items():
                    ...
                    # Basit kontrol
                    if abs(aspect_deg_diff - 0) <= orb or abs(aspect_deg_diff - 180) <= orb:
                         best_aspect_type = "Conjunction/Opposition"
                         min_orb_found = min(abs(aspect_deg_diff - 0), abs(aspect_deg_diff - 180)) % 180
    ...
    try:
        ...
        if isinstance(birth_time, str):
            try:
                ...
                logger.warning(f"Doğum saati '{birth_time}' işlenirken hata: {str(e)}. Varsayılan 12:00:00 kullanılıyor.")
                time_obj = time(12, 0, 0)
                    ...
                    try:
                        ...
                        try:
                            ...
                            try:
                                ...
                                try:
                                    # Sadece HH formatı
                                    transit_dt_time_part = datetime.strptime(transit_time_str, "%H").time()
                                except ValueError:
                        ...
                        logger.warning(f"Transit saati '{transit_time_str}' işlenirken hata: {str(e)}. Varsayılan 12:00:00 kullanılıyor.")
                        transit_dt_time_part = time(12, 0, 0)
                    transit_dt = datetime.combine(transit_dt_date_part, transit_dt_time_part)
        ...
        transit_data = calculate_transit_data(transit_dt, transit_lat, transit_lon, elevation_m)
        ...
        progression_data = calculate_progression_data(birth_dt, transit_dt, latitude, longitude, natal_planet_positions)
        ...
        solar_return_chart_data = calculate_solar_return_chart(birth_dt, transit_dt, latitude, longitude)
        result["solar_return_chart"] = solar_return_chart_data

        # 4.2 Lunar Return Haritası
        lunar_return_chart_data = calculate_lunar_return_chart(birth_dt, transit_dt, latitude, longitude)
...
def calculate_transit_data(transit_dt, latitude, longitude, elevation_m=0):
    ...
    try:
        ...
        transit_positions, transit_houses_data = get_transit_positions(transit_dt, latitude, longitude)
...
def calculate_progression_data(birth_dt, transit_dt, latitude, longitude, natal_planet_positions):
    ...
    try:
        ...
        solar_arc_progressions = get_solar_arc_progressions(birth_dt, transit_dt, natal_planet_positions)
        return {
...
def calculate_harmonic_data(birth_dt, natal_celestial_positions):
    ...
    try:
        deep_harmonic_analysis = calculate_deep_harmonic_analysis(birth_dt, natal_celestial_positions)
